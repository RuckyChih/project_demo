...
t = time.localtime()
# on_date = (pd.to_datetime((f'{t.tm_year}-{t.tm_mon}-{t.tm_mday}')) + pd.Timedelta(-1 , 'day')).date().strftime('%Y-%m-%d')
...
and coalesce(case when event_name = 'view_search_results' and screen_name = 'category_tab' and e.device = 'iphone' then coalesce(cardinality(tid_array),0) else 9999 end) >6
...
df_raw['filter_dict'] = df_raw.filter_dict.map(lambda x:list(filter(lambda w: w is not None , x)))
